...
// A copy of the data already in the entry (possibly collided). It is read from the entry together. `probe` may be racy,
// resulting in inconsistent data. After probe returns the copy, it is now local/safe.
struct TTData {
    Move  move;
    Value value, eval;
    Depth depth;
