...
Value Eval::evaluate(const Position& pos) {
  ...
  {
       int complexity;
       int scale      = 1104 + 102 * pos.non_pawn_material() / 5120;
       Color stm      = pos.side_to_move();
       Value optimism = pos.this_thread()->optimism[stm];
       Value psq      = (stm == WHITE ? 1 : -1) * eg_value(pos.psq_score());
       Value nnue     = NNUE::evaluate(pos, true, &complexity);     // NNUE

       complexity = (137 * complexity + 141 * abs(nnue - psq)) / 256;
       optimism = optimism * (253 + complexity) / 256;
       v = (nnue * scale + optimism * (scale - 867)) / 1024;
