...
inline bool use_mating_eval(const Position& pos, int abs_material_ev) {
    constexpr float advantageKRK = float(RookValue - 1) / (TwoKings + RookValue); // -1 to ensure the following float equality holds
    return float(abs_material_ev) / (TwoKings + pos.non_pawn_material()) >= advantageKRK;
}

int mating_eval(const Position& pos, int material_ev) {
