...
inline bool use_mating_eval(int abs_material_ev) {
    return abs_material_ev >= RookValue + BishopValue + PawnValue;
}
...
static int mating_eval(const Position& pos, int material_ev) {
    ...
    int bonus = 4 * PawnValue / (king_mobility(pos, loser) + 1);
...
Value Eval::evaluate(const Eval::NNUE::Network&     network,
    ...
    if (use_mating_eval(std::abs(material_ev)))
