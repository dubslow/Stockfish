...
void TTEntry::save(
  Key k, Value v, bool pv, Bound b, Depth d, Move m, Value ev, uint8_t curr_generation) {
    ...
    if (b == BOUND_EXACT || uint16_t(k) != key16
        || d - DEPTH_NONE + 2 * pv > depth8 - 4 - 8 * (is_valid(value16) && is_decisive(value16))
...
void TranspositionTable::resize(size_t mbSize, ThreadPool& threads) {
    ...
    table = static_cast<TTEntry*>(aligned_large_pages_alloc(clusterCount * sizeof(TTEntry)));
...
int TranspositionTable::hashfull(int maxAge) const {
    ...
    for (int i = 0; i < 1000; ++i)
            cnt += table[i].is_occupied()
                && table[i].relative_age(generation8) <= maxAge;
