...
void TranspositionTable::write(
  Key k, Value v, bool pv, Bound b, Depth d, Move m, Value ev) const {
    ...
    // Otherwise, an entry's value is its depth minus 12 times its relative age.
    TTEntry* worst = cluster; // == &cluster[0]
    for (int i = 1; i < ClusterSize; ++i)
        if (worst->depth8 - 12 * worst->relative_age(generation8)
            > cluster[i].depth8 - 12 * cluster[i].relative_age(generation8))
            worst = &cluster[i];
