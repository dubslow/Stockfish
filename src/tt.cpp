...
// to be replaced later. The replaceability of an entry is calculated as its depth
// minus 8 times its relative age.
std::tuple<bool, TTData, TTWriter> TranspositionTable::probe(const Key key) const {
    ...
    for (int i = 1; i < ClusterSize; ++i)
        if (replace->depth8 - 32 * replace->relative_age(generation8)
            > tte[i].depth8 - 32 * tte[i].relative_age(generation8))
            replace = &tte[i];
