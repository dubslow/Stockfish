  ...
  Value search(Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode) {
    ...
    if (!rootNode)
    {
        ...
        if (   Threads.stop.load(std::memory_order_relaxed)
            || pos.is_draw(ss->ply)
            || ss->ply >= MAX_PLY)
            return (ss->ply >= MAX_PLY && !ss->inCheck) ? evaluate(pos).first
    ...
    else if (ss->ttHit)
    {
        ...
        if (eval == VALUE_NONE)
        {        
             std::tie(eval, complexity) = evaluate(pos);
             ss->staticEval = eval;
        }
    ...
    else
    {
        if (ss->ttHit)
        {
            ...
            if ((ss->staticEval = bestValue = tte->eval()) == VALUE_NONE)
                ss->staticEval = bestValue = evaluate(pos).first;
            ...
            (ss-1)->currentMove != MOVE_NULL ? evaluate(pos).first
                                             : -(ss-1)->staticEval;
