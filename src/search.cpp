...
moves_loop:  // When in check, search starts here
    ...
    else if (prevSq != SQ_NONE)
    {
      ...
      {
        int bonus = (334 * (depth > 5) + 343 * (PvNode || cutNode)
                     + 869 * ((ss - 1)->moveCount > 9));

        thisThread->captureHistory[pos.piece_on(prevSq)][prevSq][pos.captured_piece()] << stat_bonus(depth) * bonus / 1600;
      }
    }
