...
namespace {
  ...
  Value search(Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode) {
    ...
    else if (depth <= 6 && eval < beta && eval > (alpha + 3*beta)/4)
    {
        eval = qsearch<PvNode ? PV : NonPV>(pos, ss, eval - 200 + 6*depth*depth, beta); // Narrow the window, with some margin (more leafy = more margin)
        if (eval >= beta)
            return eval;
    }
