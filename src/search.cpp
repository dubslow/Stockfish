...
int B1=116, B2=115, B3=186, B4=121, B5=64, B6=137;
auto BRange = [](int b){return std::pair<int, int>(-b*4, b*4);};
TUNE(SetRange(BRange), B1, B2, B3, B4, B5, B6);
int P2=14144, P3=115, P4=81;
TUNE(SetRange(0, 20000), P2);
TUNE(SetRange(0, 500), P3, P4);
...
moves_loop:  // When in check, search starts here
    ...
    else if (prevSq != SQ_NONE)
    {
      ...
      {
        int bonus = (B1 * (depth > 5) + B2 * (PvNode || cutNode)
                     + B3 * ((ss - 1)->statScore < -P2) + B4 * ((ss - 1)->moveCount > 9)
                     + B5 * (!ss->inCheck && bestValue <= ss->staticEval - P3)
                     + B6 * (!(ss - 1)->inCheck && bestValue <= -(ss - 1)->staticEval - P4));
