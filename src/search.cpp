      ...
      if (ss->ply < thisThread->rootDepth * 2)
      {
          ...
          if (   !rootNode
           ...
           /* &&  ttValue != VALUE_NONE Already implicit in the next condition */
              ...
              &&  tte->depth() >= depth - 3)
          {
              bool predictNonSing = (ss->cutoffCnt >  2); // sscC is weakly correlated with nonsingularity (using dbg_mean, about 60% vs 45% at depth 25)
              Value singularBeta = ttValue - (3 + (ss->ttPv && !PvNode)) * depth;
              Depth singularDepth = std::max((depth - 1) / 2 - 2 * predictNonSing, 1);
