...
bool Search::Worker::iterative_deepening() {
           ...
           && !(limits.depth && mainThread && rootDepth >= limits.depth))
    {
        ...
        for (pvIdx = 0; pvIdx < multiPV; ++pvIdx)
        {
            ...
            // Here `delta` is a radius, whereas other uses in the codebase refer to a diameter
...
moves_loop:  // When in check, search starts here
    ...
    while ((move = mp.next_move()) != Move::none())
    {
        ...
        //
        // In detail: we do a reduced search on the position excluding the ttMove, and
        // if the result is lower than ttValue minus a margin, then we extend the
        // ttMove. If however the excluded search fails high, then the ttMove is
        // non-singular and we may instead do some pruning or reduction.
        //
        ...
        // Step 18a. Compute late moves reduction (LMR) (or possibly extension)
        ...
        // Step 18b. Apply the computed LMR
        ...
        // If we have an alternative move equal in value to the current bestmove, promote
