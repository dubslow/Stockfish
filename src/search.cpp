...
moves_loop: // When in check, search starts here
    ...
    while ((move = mp.next_move(moveCountPruning)) != MOVE_NONE)
    {
      ...
      if (ss->ply < thisThread->rootDepth * 2)
      {
          ...
          int depthBound =   thisThread->previousDepth <= 1  ? 8 // At higher depths, looser bound = more SEs
                           : thisThread->previousDepth <= 3  ? 7
                           : thisThread->previousDepth <= 6  ? 6
                           : thisThread->previousDepth <= 13 ? 5
                           : thisThread->previousDepth <= 26 ? 4
                           : thisThread->previousDepth <= 53 ? 3
                           :                                   2;
