...
moves_loop:  // When in check, search starts here
    ...
    if (ttData.depth >= depth - 4 && !is_decisive(beta) && is_valid(ttData.value) && !is_decisive(ttData.value))
    {
        if ((ttData.bound & BOUND_LOWER) && ttData.value >= probCutBeta)
            return probCutBeta;
        else if (ttData.bound & (ttData.value >= probCutBeta ? BOUND_UPPER : BOUND_LOWER))
            ttWriter.penalize(1);
    }
