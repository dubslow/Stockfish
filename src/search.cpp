...
Value Search::Worker::search(
  Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode) {
    ...
    if (PvNode && !ttMove)
        depth -= 4 - (thisThread->correctionHistory[us][pawn_structure_index<Correction>(pos)] < -200)
                   - (thisThread->correctionHistory[us][pawn_structure_index<Correction>(pos)] < -400);
