...
Value Search::Worker::search(
  Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode) {
       ...
       )
    {
      ...
      {
          cutNode = cutNode && !(ttValue < alpha - std::max(270 - 10*depth, 30)); // what was a failhigh is suddenly a faillow?
          cutNode = cutNode ||  (ttValue > beta  + std::max(270 - 10*depth, 30)); // what was a faillow is suddenly a failhigh?
      }
    }
